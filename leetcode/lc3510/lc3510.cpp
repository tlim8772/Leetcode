...
// use a special segment tree
// tree[p] contains for [s, e], the leftmost index and value, rightmost index and value, best (mininum) adj pair sum
// and number of inversions
// the leftmost index may not be s, as index s, s + 1 ... can be deleted
// to merge left child and right child, see Store::merge, not that hard

// to pass time limit, use array<T, size> instead of vector, if size of list is known at compile time
...
int main() {
    ...
    vector<int> arr = {999999999, 999999998, 999999997, 999999996, 999999995};
    cout << sol.minimumPairRemoval(arr) << endl;
}
